package com.fengeee.car.owner.filter;
...
public class MisDubboProviderLogFilter implements Filter {
    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) {
        if(StringUtils.isEmpty(RpcContext.getContext().getAttachment(BaseConstant.TRACE_ID))){
            RpcContext.getContext().setAttachment(BaseConstant.TRACE_ID, StringUtils.replace(UUID.randomUUID().toString(), "-", ""));
        }
        ThreadContext.put(BaseConstant.TRACE_ID, RpcContext.getContext().getAttachment(BaseConstant.TRACE_ID));
        ...
        if (result.hasException()) {
            ...
            if (resultJson.length() > BaseConstant.LOG_MAX_LENGTH) {
                log.info("{}端,接口:{},方法:{},耗时:{},请求结果:{},结果长度:{}", Constants.PROVIDER, invocation.getInvoker().getInterface(), invocation.getMethodName(), invokeTime, resultJson.substring(0, BaseConstant.LOG_MAX_LENGTH), resultJson.length());
            } else {
