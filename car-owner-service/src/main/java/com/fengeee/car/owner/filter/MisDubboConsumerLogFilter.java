...
public class MisDubboConsumerLogFilter implements Filter {
    ...
    private void log(long start, Result result, Invocation invocation) {
        ...
        if (result.hasException()) {
            ...
            if (resultJson != null && resultJson.length() > BaseConstant.LOG_MAX_LENGTH) {
                log.info("s_dubbo:{}ms {}.{}: argLen:{} error:{}", invokeTime, invocation.getInvoker().getInterface().getSimpleName(), invocation.getMethodName(), resultJson.length(), resultJson.substring(0, BaseConstant.LOG_MAX_LENGTH));
            } else {
            ...
            if (resultJson.length() > BaseConstant.LOG_MAX_LENGTH) {
                log.info("s_dubbo:{}ms {}.{}: argLen:{} ret:{}", invokeTime, invocation.getInvoker().getInterface().getSimpleName(), invocation.getMethodName(), resultJson.length(), resultJson.substring(0, BaseConstant.LOG_MAX_LENGTH));
            } else {
